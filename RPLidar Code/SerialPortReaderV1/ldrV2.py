...
class AnalogPlot:
  ...
  def __init__(self, strPort, maxLen):
      ...
      # start timer
      print("--- %s seconds at serial port---" % (time.time() - start_time))
  ...
  def update(self, frameNum, a0, a1):
      try:
          line = self.ser.readline()
          # start timer
          print("--- %s seconds at the update---" % (time.time() - start_time))
...
def main():
  ...
  anim = animation.FuncAnimation(fig, analogPlot.update,
                                 ...
                                 interval=50)
  # start timer
  #print("--- %s seconds finshed plot---" % (time.time() - start_time))
...
if __name__ == '__main__':
  ...
  # python3 ldr.py --port /dev/ttyACM0
